	...
	if(isset($_POST['username']) && isset($_POST['password']) && isset($_POST['fullname']) && isset($_POST['age'])){
		...
		if ($username!='' && $password!='' && $fullname!='' && $age!=''){
			...
			if(mysqli_num_rows($check) == 0){
			    $result = mysqli_query($con,"INSERT INTO users (username, fullname, age, RSApass, hashPass, role) VALUES ('$username', '$fullname', $age, '$RSApass', '$hashPass', '0')");
				$query = mysqli_query($con,"SELECT fullname FROM `users` WHERE username='$username'");
