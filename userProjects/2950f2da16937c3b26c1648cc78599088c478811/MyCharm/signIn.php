...
<?php
    if(isset($_POST['username']) && isset($_POST['password'])){
        ...
        if ($username!='' && $password!=''){
            ...
            $query = mysql_query("SELECT fullname, age FROM `users` WHERE username='$username' and hashPass='$hashPass'");
            $row = mysql_fetch_array($query);

            if(!empty($row['fullname']) && !empty($row['age'])){
                $admin_hashPass_query = mysql_query("SELECT hashPass FROM `users` WHERE username='admin' and role='1' and id='1'");
                $row2 = mysql_fetch_array($admin_hashPass_query);
